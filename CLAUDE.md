...
- **Evening Debrief** (6:30 PM): Summarizes the day
  ...
  - Time-sensitive items only (overdue assignments/tasks, or due within 48h) — never a guilt-trip list of everything still pending; a light day gets a reassuring line instead
  - Tomorrow's outlook
...
- Scheduled jobs (assignment due, overdue tasks, tasks due tomorrow, AP countdown, meeting reminders, trash reminder, weather alerts, stock alerts) and the morning briefing all push through this same fan-out; each gates on `_notifications_configured()` rather than a single channel. There is deliberately no activity/idle nudge — Jarvis never pushes about incomplete work unless something is actually overdue or due soon
...
- Multi-photo albums arrive as **separate updates** sharing a `media_group_id` (and only the first carries the caption), so `_telegram_queue_album()` buffers them in-memory for 3s and fires a single turn with all the images — otherwise three photos of one invite would mean three Jarvis turns and three duplicate calendar events. Relies on the app running as one process
